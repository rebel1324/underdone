...
function GM:PlayerCanSeePlayersChat(strText, bTeamOnly, pListener, pSpeaker)
	...
	local intChatDistance = 70000
	if tblText[#tblText] == "!" && #tblText > 1 then intChatDistance = 70000 end
	if tblText[#tblText] == "@" && #tblText > 1 then intChatDistance = 70000  if !pListener:IsInSquad(pSpeaker) or !pSpeaker:IsInSquad(pListener) then return false end end
	if pListener:GetPos():Distance(pSpeaker:GetPos()) >= intChatDistance then return false end --Too Far
	return true --All good :)
end
