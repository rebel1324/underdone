...
function Entity:CanBurden( add )
		local total = 0
		for item, num in pairs( self.Data.Inventory ) do
				local tblItemTable = ItemTable(item)
				total = total + ( tblItemTable.Weight * num )
		end
		
		if total + add <= self:GetMaxWeight() then return true end
end
