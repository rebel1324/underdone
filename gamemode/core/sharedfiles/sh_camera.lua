...
function Player:GetIdealCamPos()
	...
	if intEditorRadiants or intEditorDistance then
		...
		tracedata.start = vecPosition + Vector(0, 0, 25)
		tracedata.endpos = vecPosition + (self:EyeAngles():Right() * intDistance2) + Vector(0, 0, 25) 
		tracedata.filter = self.Owner
		local trace = util.TraceLine(tracedata) 
		intDistance2 = trace.HitPos:Distance(tracedata.start) 
		
		local ep = (self:EyeAngles():Forward() * -intDistance) + Vector(0, 0, 25) + (self:EyeAngles():Right() * intDistance2)
		
		vecPosition = vecPosition + ep
	end
	return vecPosition
end
...
if SERVER then
	...
	function GM:CalcView(plyClient, vecOrigin, angAngles, fovFieldOfView)
		...
		GAMEMODE.CammeraPosition = LerpVector(GAMEMODE.CammeraDelta, GAMEMODE.CammeraPosition, client:GetIdealCamPos() + Vector( 0, 0, 0 ) )
