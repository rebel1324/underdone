...
local crosshair = surface.GetTextureID( "icons/crosshair_ud" )
...
function GM:HUDPaint()
	...
	if GAMEMODE.ConVarShowCrossHair:GetBool() then
		...
		tracedata.endpos = vecPosition + vecDir * 10000
		...
		if trace.Entity:IsValid() then
			varsize = math.sin( CurTime()* 20 ) * 5
		end
		
		local size = realsize + varsize
